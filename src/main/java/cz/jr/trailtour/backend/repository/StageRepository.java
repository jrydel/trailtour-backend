...
public class StageRepository extends MysqlRepository {
    ...
    public List<Stage> getAll() throws SQLException {
        return selectList("SELECT a.id, a.country, a.number, a.name, a.type, a.distance, a.elevation, a.latitude, a.longitude, COUNT(b.stage_id) AS activities FROM trailtour.stage a LEFT JOIN trailtour.result b ON a.id = b.stage_id GROUP BY a.id", new Object[]{}, rs -> {
            Stage stage = new Stage();
