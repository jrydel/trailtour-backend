...
public class StageRepository extends BaseRepository {
    ...
    public StageData get(String database, int number) throws SQLException {
        ...
        return selectObject("SELECT a.name, a.type, a.distance, a.elevation, a.url, a.strava_url, a.strava_data, a.mapycz_url, COUNT(*) AS activities FROM " + database + ".stage a LEFT JOIN " + database + ".athlete_result b ON a.number = b.stage_number WHERE a.number = ? AND b.timestamp = ? GROUP by a.number", new Object[]{number, java.sql.Timestamp.valueOf(update)}, rs -> {
